...
def load_universe(file_path="universe.txt", min_price=1.0, max_price=1500.0, exchange="all"):
    ...
    for i in range(0, len(all_tickers), batch_size):
        ...
        try:
            ...
            if isinstance(data.columns, pd.MultiIndex):
                ...
                for t in batch:
                    try:
                        price = closes if isinstance(closes, (int, float, np.float64)) else closes.get(t, np.nan)
                        if pd.isna(price):
                            skipped_tickers.append(t)
                        elif min_price <= price <= max_price:
                            valid_tickers.append(t)
    ...
    if skipped_tickers:
        st.warning(f"Skipped {len(skipped_tickers)} tickers due to download errors or price filters: {', '.join(skipped_tickers[:20])}{'...' if len(skipped_tickers) > 20 else ''}")

    valid_tickers = [t.replace(".", "-") for t in valid_tickers]
    p.write_text("\n".join(valid_tickers))
    st.info(f"Universe loaded: {len(valid_tickers)} tickers from {', '.join(exchanges).upper()} between ${min_price} and ${max_price}")

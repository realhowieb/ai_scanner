...
try:
    ...
    # --- Fallback implementations for market heat if ai_scanner.data.fetch is unavailable ---
    try:
    ...
    def _yahoo_to_df(items: List[Dict]) -> pd.DataFrame:
        ...
        # Light formatting if available
        for col in ["price", "change", "change_%"]:
    ...
    def _fetch_predefined_screener(scr_id: str, count: int = 25) -> pd.DataFrame:
        """Call Yahoo Finance predefined screener JSON (no bs4/lxml required)."""
    ...
    def _fetch_quotes(symbols: List[str]) -> pd.DataFrame:
        """Enrich basic symbols with real-time quote fields via Yahoo quote API."""
        if _requests is None or not symbols:
            return pd.DataFrame()
        all_items: List[Dict] = []
        # Yahoo quote endpoint supports batching; keep chunks modest
    ...
    def _fetch_trending(region: str = "US", count: int = 25) -> pd.DataFrame:
        ...
        # 1) Fast path: Yahoo predefined screener for trending tickers
        df = _fetch_predefined_screener("trending_tickers", count=count)
        if isinstance(df, pd.DataFrame) and not df.empty:
            return df

        # 2) Backup: call v1 trending endpoint and enrich via quote API
        def _try_trending(base: str) -> List[str]:
            ...
            try:
                ...
                # Some regions/variants may return strings instead of dicts
                symbols: List[str] = []
                for q in raw:
        ...
        # 3) Last resort: Most Active to avoid empty UI
        return _fetch_predefined_screener("most_actives", count=count)

    # Bind fallbacks only if original imports failed
    if fetch_hot_stocks is None:
        def fetch_hot_stocks(count: int = 25) -> pd.DataFrame:  # type: ignore
            # Use Day Gainers as proxy for "Hot Stocks"
            return _fetch_predefined_screener("day_gainers", count=count)

    if fetch_most_active_stocks is None:
        def fetch_most_active_stocks(count: int = 25) -> pd.DataFrame:  # type: ignore
            return _fetch_predefined_screener("most_actives", count=count)

    if fetch_trending_stocks is None:
        def fetch_trending_stocks(region: str = "US", count: int = 25) -> pd.DataFrame:  # type: ignore

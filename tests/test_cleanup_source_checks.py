...
class CleanupSourceChecks(unittest.TestCase):
    ...
    def test_app_runtime_helpers_are_extracted_from_app(self):
        ...
        self.assertLess(len(app_source.splitlines()), 840)
    ...
    def test_result_helpers_are_extracted_from_results_ui(self):
        ...
        self.assertLess(len(results_source.splitlines()), 795)
        ...
        self.assertIn("def _can_use_background_gradient", results_source)
        self.assertIn("if can_use_gradient and \"BreakoutScore\" in table_df.columns", results_source)
        self.assertIn("if can_use_gradient and \"RS_Rank\" in table_df.columns", results_source)
        self.assertIn("except (ImportError, RuntimeError, TypeError, ValueError)", results_source)
    ...
    def test_auth_lockout_helpers_are_extracted_from_auth_ui(self):
        ...
        self.assertLess(len(auth_source.splitlines()), 740)
